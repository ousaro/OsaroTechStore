...
export const buildLoginUserUseCase = ({ authUserRepository, tokenService }) => {
  ...
  return async ({ email, password }) => {
    ...
    const passwordMatches = user && (await authUserRepository.comparePassword(command.password, user.password));
    if (!user || !passwordMatches) throw new Error("Email or Password are not correct");
