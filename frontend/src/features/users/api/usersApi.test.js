...
test("getMe fetches /users/me with token", async () => {
  ...
  const repo = createHttpUserRepository({
...
test("updateMe sends PUT to /users/me with body and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/users/me", {
    method: "PUT",
    body: { firstName: "Jane" },
    token: "tok-1",
  });
...
test("updatePassword sends PUT to /users/me/password with body and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/users/me/password", {
    method: "PUT",
...
test("toggleFavorite sends PUT to /users/me/favorites/:productId with action and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/users/me/favorites/p1", {
    method: "PUT",
    body: { action: "add" },
    token: "tok-1",
  });
...
test("updateById sends PUT to /users/:id with body and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/users/u1", {
    method: "PUT",
    body: { isAdmin: true },
    token: "tok-1",
  });
