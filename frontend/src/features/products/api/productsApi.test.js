...
test("getAll fetches /products with query params and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/products?category=Phones&status=active", {
    token: "tok-1",
  });
});
...
test("update sends PUT to /products/:id with patch and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/products/p1", {
    method: "PUT",
    body: { price: 99 },
    token: "tok-1",
  });
...
test("addReview sends POST to /products/:id/reviews with body and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/products/p1/reviews", {
    method: "POST",
