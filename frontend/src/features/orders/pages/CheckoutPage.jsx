...
export function CheckoutPage({ ordersInputPort, paymentsInputPort }) {
  ...
  const placeOrder = async (e) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(err, "Could not start checkout. Please review your order and try again.")
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Delivery details, item summary, and protected payment in one clear step.
          </p>
        </div>
      </div>
      <div className="checkout-banner">
        <div className="checkout-banner-item">
          <FiTruck size={18} /> Fast dispatch on in-stock items
        </div>
        <div className="checkout-banner-item">
          <FiMapPin size={18} /> Ship to your saved address or update it here
        </div>
        <div className="checkout-banner-item">
          <FiShield size={18} /> Stripe-secured checkout session
        </div>
      </div>
      ...
      <div className="grid min-w-0 items-start gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <div className="card checkout-summary">
          ...
          {cartItems.map((i) => (
            <div key={i.id} className="checkout-line">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[13px] font-semibold leading-snug">{i.name}</div>
                <div className="text-xs text-ink-muted">Qty: {i.quantity}</div>
              </div>
              <div className="ml-auto text-sm font-bold">
                {i.price.currency} {(i.price.amount * i.quantity).toFixed(2)}
              </div>
            </div>
          ))}
          ...
          <p className="checkout-summary-note">
            You will be redirected to a secure payment page after placing the order.
          </p>
        </div>
      </div>
    </div>
