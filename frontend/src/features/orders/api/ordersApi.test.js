...
test("create sends POST to /orders with body and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/orders", { method: "POST", body: { items: [] }, token: "tok-1" });
});
...
test("update sends PUT to /orders/:id with body and token", async () => {
  ...
  expect(mockHttpClient).toHaveBeenCalledWith("/orders/o1", { method: "PUT", body: { orderStatus: "shipped" }, token: "tok-1" });
});
